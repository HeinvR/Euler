...
def getVerticalGrid():
    ...
    for column in range(0,20):
        verticallist = []
        for row in grid:
            verticallist.append(row[column])
        verticalgrid.append(verticallist)
    return verticalgrid
...
def greatestHorizontalProduct():
    ...
    for row in grid:
        rowresult = []
        for i in range(0,17):
            subrowresult = math.prod(row[i:i+4])
            rowresult.append(subrowresult)
        gridresult.append((max(rowresult)))
    return(max(gridresult))

def greatestVerticalproduct():
    gridresult = []
    for row in verticalgrid:
        rowresult = []
        for i in range(0,17):
            subrowresult = math.prod(row[i:i+4])
            rowresult.append(subrowresult)
...
def greatestDiagonalProduct(grid):
    indexlist = []
    ...
    for row in range(0,len(grid)):
        for column in range(0,len(grid[row])):
            sublist = []
            for i in range(0,4):
                if len(grid) > (row + i) and len(grid[row]) > (column + i):
                    sublist.append((row + i, column + i))
                else:
                    break
            if len(sublist) == 4:
    ...
    for row in reversed(range(0,len(grid))):
        for column in range(0,len(grid[row])):
            sublist = []
            for i in range(0,4):
                if (row -i) > 0 and len(grid[row]) > (column + i):
                    sublist.append((row - i, column + i))
                else:
                    break
            if len(sublist) == 4:
    ...
    for item in indexlist:
        items = []
        for x,y in item:
            items.append(grid[x][y])
        # resultlist.append(items)
        resultlist.append(math.prod(items))
...
verticalgrid = getVerticalGrid()
greatestDiagonalProduct(grid)
verticalproduct = greatestVerticalproduct()
horizontalproduct = greatestHorizontalProduct()

print(verticalproduct)
print(horizontalproduct)
